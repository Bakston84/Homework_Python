...
def printTable(operation, num_rows, num_сolumns):
    array = [[operation(i, j) for i in range(1, num_rows + 1)] for j in range(1, num_сolumns + 1)]
    for i in array:
        print(*[f'{x:>3}' for x in i])

line = int(checkingInput('Введите количество строк > '))
columns = int(checkingInput('Введите количество столбцов > '))
printTable(lambda x, y: x * y, line, columns)